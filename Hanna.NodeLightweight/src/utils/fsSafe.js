...
async function readJsonlAsync(file, limit = 50) {
  if (!fs.existsSync(file)) return [];
  const buffer = [];
  try {
    ...
    for await (const line of rl) {
      ...
      try {
        ...
        buffer.push({ parse_error: true, raw: line });
        if (buffer.length > limit) buffer.shift();
      }
    }
  } catch (error) {
    console.error(`Error leyendo stream de ${file}:`, error.message);
  }
  return buffer;
}
...
module.exports = { ensureDir, ensureFile, appendJsonl, readJsonl, readJsonlAsync, walkFiles };
